...
**Cross-device credential sync was NOT actually wired in the original S3 PR** (the `S3Settings` comment "credentials live here like WebDAV's" was aspirational). Wired in PR readest/readest#5051, MERGED to main (squash commit `36ea007e1`), mirroring WebDAV [[webdav-credential-sync-4810]]: `adapters/settings.ts` `SETTINGS_WHITELIST` += `s3.endpoint/region/bucket/accessKeyId/secretAccessKey`, `SETTINGS_ENCRYPTED_FIELDS` += `s3.accessKeyId/secretAccessKey`; `replicaSettingsSync.ts` `mergeSettings` gains `if (patch.s3)` deep-merge (WITHOUT it a pull clobbers device-local s3.enabled/deviceId — the #4810 trap; VERIFIED mergeSettings is STILL top-level-shallow + per-group deep-merge cases, contra a subagent's claim it was per-path); `backupService.ts` BACKUP_SETTINGS_BLACKLIST += s3 device-local trio + BACKUP_SETTINGS_CREDENTIAL_FIELDS += s3 secrets. Deferred: SyncCategoriesSection credentials-description copy (still lists WebDAV not S3 — 33-locale i18n migration). Parity gap noted: webdav.username/password are NOT in BACKUP_SETTINGS_CREDENTIAL_FIELDS (WebDAV creds still plaintext in backup zips).
